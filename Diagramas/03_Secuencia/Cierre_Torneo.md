...
sequenceDiagram
    ...
    alt Formato == Liga (Todos contra todos)
        ...
        API->>API: Consolida la tabla y determina posiciones
        
        alt Existe un Empate Absoluto en el primer puesto
            API-->>UI: HTTP 409 Conflict (JSON con Alerta de Empate Absoluto)
            UI->>UI: Despliega modal de Resolución Manual (Sorteo / Partido Extra)
            UI-->>Org: Solicita decisión del Organizador
            
            note right of Org: El Organizador resuelve y el flujo <br>pasa a Secuencia_Resolucion_Empate_Absoluto.md
        else Hay un Campeón definido matemáticamente
